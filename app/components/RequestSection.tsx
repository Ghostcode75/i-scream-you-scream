...
export default function RequestSection({ onSubmit, heat, truck }: RequestSectionProps) {
    ...
    <section className="animate-slideUp stagger-1">
      ...
      {modalOpen && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget && !submitted) closeModal(); }}
        >
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" />
          <div className="relative bg-[#FFF9F5] w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl max-h-[92dvh] sm:max-h-[85vh] overflow-y-auto shadow-2xl animate-slideUp overscroll-contain">
            {/* Drag handle on mobile */}
            <div className="sm:hidden flex justify-center pt-3 pb-1">
              <div className="w-10 h-1 rounded-full bg-[#D4DDE2]" />
            </div>
            {/* Modal header */}
            <div className="sticky top-0 bg-[#FFF9F5] px-5 pt-3 sm:pt-5 pb-3 flex items-center justify-between border-b border-[#E2E8EC] z-10">
              <h2 className="text-lg font-bold text-[#3A2D1E]">
                {submitted ? "Request Sent!" : AREAS.find((a) => a.id === area)?.name}
              </h2>
              {!submitted && (
                <button
                  onClick={closeModal}
                  className="w-8 h-8 rounded-full bg-[#F5F8FA] flex items-center justify-center text-[#6B7C88] hover:bg-[#E8EDF0] transition"
                >
                  <IconX size={18} />
                </button>
              )}
            ...
            <div className="px-5 py-5 pb-[max(20px,env(safe-area-inset-bottom))]">
              {/* ─── Confirmation ─── */}
              {submitted && lastReq ? (
                <div className="text-center animate-slideUp">
                  ...
                  <div className="inline-flex flex-col bg-gradient-to-br from-[#6EC6CE] to-[#5BB8C0] text-white rounded-2xl px-8 py-4 mb-5">
                    ...
                    <span className="text-4xl font-extrabold leading-none my-1">
                      ~{AREAS.find((a) => a.id === area)?.eta || 15} min
                    </span>
                    <span className="text-[11px] opacity-70">once the truck heads your way</span>
                  </div>

                  {totalItems > 0 && (
                    <div className="bg-white rounded-xl p-4 my-4 text-left text-sm">
                ...
                <div className="animate-slideUp">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="text-base font-bold text-[#3A2D1E]">What street?</h3>
                  </div>
                  ...
                  <input
                    ...
                    autoFocus
                    className="w-full px-4 py-3 border-2 border-[#E2E8EC] rounded-xl bg-white text-[#3A2D1E] text-base mb-3"
                  />
                  <input
                    type="text"
                    ...
                    className="w-full px-4 py-3 border-2 border-[#E2E8EC] rounded-xl bg-white text-[#3A2D1E] text-base mb-5"
                  />
                  <button
                    onClick={() => setModalStep(1)}
                    disabled={!street.trim()}
                    className="w-full py-3 rounded-xl bg-[#6EC6CE] text-white text-sm font-bold disabled:opacity-40 hover:bg-[#5BB8C0] transition inline-flex items-center justify-center gap-1"
                  >
                ...
                /* ─── Step 1: Treat Menu ─── */
                <div className="animate-slideUp">
                  <h3 className="text-base font-bold text-[#3A2D1E] mb-1">Pre-order treats</h3>
                    ...
                    {TREATS.map((t) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[10px] font-bold text-[#3A2D1E] text-center leading-tight min-h-[24px] flex items-center">
                            {t.name}
                          </span>
                  ...
                  {totalItems > 0 && (
                    <div className="text-center text-sm text-[#3A2D1E] bg-white rounded-xl p-3 mb-4 font-semibold border border-[#F4E8DD]">
                ...
                /* ─── Step 2: Rally + Summary + Submit ─── */
                <div className="animate-slideUp">
                  <div className="flex items-center gap-2 mb-1">
                    <IconUsers size={20} className="text-[#6EC6CE]" />
                    <h3 className="text-base font-bold text-[#3A2D1E]">Rally the block</h3>
                  </div>
                  ...
                  {/* Summary */}
                  <div className="bg-white rounded-xl p-4 mb-5 text-sm space-y-1.5">
